...
import {DEFAULT_PAGE_SIZE, MIN_ENTITIES_FOR_PAGINATION, PAGE_SIZE_OPTIONS} from "../../../../consts/default-table-values";
...
export class UsersPageComponent implements OnInit, OnDestroy {
  ...
  protected entityService = inject(UserService);
  protected configService = inject(UserConfigService);
  protected dialogService = inject(UserDialogService);
  ...
  visibleEntities = signal<AppUser[]>(this.activatedRoute.snapshot.data['entities']);
  ...
  private addEntityToView(entity?: AppUser) {
    if (entity) {
  ...
  private processUrlFragment(fragment: string) {
    ...
    if (entityType === this.entityMetadata.name) {
      ...
      switch (action) {
        case 'add':
          this.dialogService.openDialog(DialogMode.ADD, undefined, this.visibleEntities(), this.projects, this.organizations);
          break;
        case 'edit':
          if (entity) this.dialogService.openDialog(DialogMode.EDIT, entity, this.visibleEntities(), this.projects, this.organizations);
          break;
        case 'delete':
