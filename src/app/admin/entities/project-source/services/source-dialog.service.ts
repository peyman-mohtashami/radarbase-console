...
export class SourceDialogService {
  ...
  async openDialog(mode: DialogMode, entity?: AppSource) {
    if (mode !== DialogMode.ADD && !entity) return;
    await this.createDialogRef(mode, entity);
  }

  private async createDialogRef(mode: DialogMode, entity?: AppSource): Promise<MatDialogRef<SourceDialogComponent>> {
