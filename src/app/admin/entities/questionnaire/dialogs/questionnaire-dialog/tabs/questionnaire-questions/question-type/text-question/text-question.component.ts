...
export interface QuestionnaireTextQuestionForm extends Record<string, unknown> {
  id: string;
  field_name: string;
  field_type: string;
...
export class TextQuestionComponent {
  ...
  protected model = signal<QuestionnaireTextQuestionForm>({ //this.dialogData.restoredModel ??{
    ...this._question,
  ...
  toAppQuestion(model: QuestionnaireTextQuestionForm): AppQuestion {
  ...
  normalizeQuestion(question: AppQuestion) {
    const updatedQuestion: AppQuestion = {
      ...
      multi_line: question.multi_line,
