...
export class QuestionPickerDialogComponent {
  ...
  protected readonly data = inject<{questionPicker: boolean; questionIndex: number}>(MAT_DIALOG_DATA);

  protected RESERVED_VARIABLES = RESERVED_VARIABLES;

  readonly questions = this.store.selected()?.questions.filter(((q, i) => i < this.data.questionIndex));

  selectQuestion(event: MatSelectChange<AppQuestion | string>): void {
