...
export interface QuestionnaireAudioQuestionForm extends Record<string, unknown> {
  id: string;
  field_name: string;
  field_type: string;
...
export class AudioQuestionComponent {
  ...
  protected model = signal<QuestionnaireAudioQuestionForm>({ //this.dialogData.restoredModel ??{
    ...this._question,
  ...
  toAppQuestion(model: QuestionnaireAudioQuestionForm): AppQuestion {
